...
namespace config {
  ...
  int total_steps = 200000; // [безразм] общее количество шагов
  int log_period = 10000; // [безразм] период вывода в консоль
  int save_period = 2000; // [безразм] период сохранения для OVITO
  ...
  int salt_period = 3; // [безразм] каждая такая частица - соль
  ...
  // --- Геометрия системы ---
  double box_x = 100.0; // [Ангстрем] полудлина бокса по X
  double box_y = 50.0; // [Ангстрем] полуширина по Y
  double box_z = 50.0; // [Ангстрем] полувысота по Z
